...
| **마스터 관리** | 수리부품 관리 | 부품명, 부품코드, 단가, 구분(부품/공임), 적용 장비(다중) |
| **마스터 관리** | 수리 세트 | 자주 사용하는 부품 조합을 프리셋으로 등록 · 다중 선택 적용 (공임 최고가 자동 선택) |
| **AS 입고** | 일괄 입고 등록 | 공통 정보 1회 입력 + 인라인 품목별 다중 추가, 쉼표 시리얼 자동 분리 |
| **AS 수리** | 수리 기록 등록 | 부품/공임 분리 체크박스로 선택 · 수리세트 다중 적용 → 비용 자동 계산 · 수리완료 자동 상태 변경 |
| **AS 출고** | 다중 출고 처리 | 수리완료 장비 선택 → 오늘 출고 / 날짜 선택 출고 |
| **AS 이력** | 통합 이력 조회 | 제한적 부분 수정 가능 (기존 데이터 내역 조회 및 상태 정정 액션 포함) |
| **추출** | 견적서 발행 (데모) | 수리완료/출고 상태 장비만 조회 및 엑셀 폼(다스 공구실 양식)과 완전히 동일한 레이아웃 표 서식의 PDF 추출 |
| **추출** | 세금계산서 등록 | 출고 완료(SHIPPED) 장비의 세금계산서 발행 여부 마킹 (내부 조회용) |
| **툴 물류** | 통합 입출고 이력 | 기존 파이썬 앱(`inventory.db`)의 레거시 내역 통합 및 재고 조회 |
| **툴 물류** | 재고 내역 인쇄 | `ToolStockSummary` 모델을 통하여 현재 재고 현황 리스트 PDF 출력 (`actions_list` 버튼 활용) |
| **대시보드** | KPI 카드 | 수리 대기 · 이번 달 입고 · 수리 완료(미출고) · 이번 달 출고 |
| **UI/UX** | 상태별 행 색상 | 뱃지 대신 전체 행(Row)에 입고(노랑), 수리완료(초록), 출고(파랑), 폐기(빨강) 등의 색상을 적용해 직관성 극대화 |
| **UI/UX** | 네비게이션 일관성 | 다크 테마 기본 활성화 및 대시보드 중심의 빵부스러기(Breadcrumb), 검색/필터 버튼 한국어 최적화 |
...
│   ├── settings.py                   #   Unfold, DB, SIDEBAR 다중 네비게이션 설정
│   ├── urls.py                       #   URL 라우팅
│   ├── wsgi.py                       #   WSGI 설정
│   └── asgi.py                       #   ASGI 설정
├── as_app/                           # 핵심 애플리케이션 (AS 업무 관련 로직)
...
│   └── apps.py                       #   앱 설정
├── tool_inventory/                   # 물류 애플리케이션 (툴 장비 재고 입/출고)
│   ├── models.py                     #   Inventory. ToolStockSummary 등 프록시 모델 통합
│   └── admin.py                      #   격리된 ToolInventoryAdminSite를 사용한 재고 어드민 UI
├── templates/                        # 커스텀 템플릿
│   └── admin/as_app/                 #   Admin 템플릿 오버라이드
│       ├── company_tabs.html         #     업체관리 탭 네비게이션 (공통)
│       └── ...                       #     (생략)
├── requirements.txt                  # Python 의존성 패키지 (ReportLab 등 포함)
...
2. **수리 기록** — 입고된 장비의 "🔧 수리 기록 등록" 버튼 클릭 → 부품/공임 분리 체크박스에서 직접 선택하거나, 수리 세트를 다중 적용(부품 누적 · 공임 최고가 자동 선택) → 저장 시 비용 자동 계산 + 수리완료 상태 자동 변경.
3. **출고 처리** — 수리 완료된 장비를 체크박스로 선택 → "오늘 날짜 출고" 또는 "날짜 선택 출고" 액션으로 일괄 출고 처리.
4. **통합 이력** — 모든 AS 이력을 상태·업체·날짜·브랜드 등으로 필터링/검색하여 조회. 체크박스 선택 후 상태 정정 액션 사용 가능 (출고/수리/폐기 취소, 견적서/세금계산서 초기화 등).
...
| `ASHistory` | 통합 이력 조회 | 전체 (제한적 수정 가능) |
| `EstimateTicket` | 견적서 조회용 | 수리완료/출고 (양식동일 PDF 추출) |
...
- [x] 수리 세트 (RepairPreset) — 자주 사용하는 부품 조합 프리셋, 다중 선택 적용 지원 (공임 최고가 자동 선택)
...
- [x] 단가 그룹별 가격 관리 — 견적서 출력 시 업체별 단가 그룹에 따른 가격 자동 적용 반영 완료.
- [x] 견적서 출력 — 엑셀 양식(다스 공구실 양식)과 완전히 동일한 레이아웃 표 서식을 ReportLab을 이용해 PDF 원클릭 다운로드 기능 지원.
- [ ] 분해도 이미지 매핑
- [ ] 모바일 반응형 최적화
