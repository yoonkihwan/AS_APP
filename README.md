...
| **Database** | SQLite / PostgreSQL | 로컬 개발용 SQLite, 운영 배포용 PostgreSQL |
...
| **AS 이력** | 통합 이력 조회 | 입고~출고 전체 이력 읽기 전용 조회, 다양한 필터·검색 |
| **대시보드** | KPI 카드 | 수리 대기 · 이번 달 입고 · 수리 완료(미출고) · 이번 달 출고 |
| **UI/UX** | 상태 컬러 뱃지 | 입고(노랑), 수리완료(초록), 출고(파랑), 폐기(빨강) 등 단위별 명확한 시각화 |
...
- [x] UI/UX 고도화 — 상태 컬러 뱃지 적용, 대시보드 네비게이션 일치 및 한국어 텍스트 최적화
