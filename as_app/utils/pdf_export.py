...
def generate_custom_pdf_estimate(custom_data_list) -> io.BytesIO:
    """커스텀 딕셔너리 데이터를 기반으로 다스 공구실 견적서 PDF 생성"""
    _register_fonts()
    buffer = io.BytesIO()
    ...
    for idx, data in enumerate(custom_data_list):
        if idx > 0:
            from reportlab.platypus import PageBreak
            elements.append(PageBreak())

        company_name = data.get("company_name", "업체미정")
        model_name = data.get("model_name", "품목미정")
        ...
        try:
            nego_price = int(data.get("nego_price", 0))
        except (ValueError, TypeError):
            nego_price = 0
        ...
        for part in parts:
            ...
            try:
                part_unit_price = int(part.get("unit_price", 0))
                part_amount = int(part.get("amount", 0))
            except (ValueError, TypeError):
                part_unit_price = 0
                part_amount = 0
            part_remark = part.get("remark", "")

            table_data.append([
                Paragraph(part_name, cell_style),
                Paragraph(part_code, cell_center),
                Paragraph(f"{part_qty} EA", cell_center),
                Paragraph(f"{part_unit_price:,}", cell_right),
                Paragraph(f"{part_amount:,}", cell_right),
                Paragraph(part_remark, cell_style),
            ])
